...
for (const gameId of ['game5', 'game6', 'game7']) {
  test(`${gameId} có live region auto-save đầy đủ`, async ({ page }) => {
    await page.addInitScript(() => localStorage.clear());
    await page.goto(`/${gameId}.html`, { waitUntil: 'domcontentloaded' });

    await expect(page.locator('script[data-bh-games5-7-autosave]')).toBeAttached();
    await page.waitForFunction(() => Boolean(window.BongGames57Autosave));
