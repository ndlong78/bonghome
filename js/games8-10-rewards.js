(function (root, factory) {
...
})(typeof window !== 'undefined' ? window : globalThis, function createGames8To10Rewards(progress, rewards, root) {
  'use strict';

  root = root || null;
  if (!progress || !rewards) throw new Error('Games 8-10 rewards require progress and rewards modules');

  const REWARD_CONFIG = Object.freeze({
    game8: Object.freeze({ stars: 2, stickerId: 'explorer-compass', stickerName: 'La bàn khám phá', stickerIcon: '🧭' }),
    game9: Object.freeze({ stars: 2, stickerId: 'story-book', stickerName: 'Cuốn sách kể chuyện', stickerIcon: '📖' }),
    game10: Object.freeze({ stars: 2, stickerId: 'happy-note', stickerName: 'Nốt nhạc vui', stickerIcon: '🎵' })
  });
  ...
  function rewardLines(detail) {
    ...
    if (detail.newBadge) lines.push('🏆 Huy hiệu mới: Bé chinh phục');
    return lines;
  }
  ...
  function completeGame(gameId, result) {
    ...
    const completedAll = ['game8', 'game9', 'game10'].every((id) => Number(byGame[id]) > 0);
    const badgeResult = completedAll
      ? rewards.unlockBadge('conqueror-kid', { source: 'games8-10-complete' })
      : { duplicate: true };
