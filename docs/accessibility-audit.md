...
- Phần thưởng Game 1 khai báo trực tiếp `role="status"`, `aria-live="polite"` và `aria-atomic="true"` trước khi chèn nội dung; cập nhật phần thưởng không làm thay đổi focus hiện tại.
- Phần thưởng Game 2–4 khai báo trực tiếp đầy đủ `role="status"`, `aria-live="polite"` và `aria-atomic="true"` trước khi chèn nội dung; cập nhật phần thưởng không làm thay đổi focus hiện tại.
...
- Nhãn thống kê nhỏ trong Game 1–10 và chữ hướng dẫn dùng token `--bh-color-muted: #806987`; màu này đạt khoảng 4.62:1 trên nền kem và 4.9:1 trên nền trắng.
- Hộp tiếp tục Game 1 dùng màu chữ phụ qua token chung; hai nút tiếp tục/chơi lại đạt lần lượt khoảng 5.66:1 và 5.1:1 trên nền pastel tương ứng.
- Câu khen trong hộp hoàn thành Game 1–10 dùng token chữ phụ chung và đạt khoảng 4.9:1 trên nền trắng.
- Thông báo lưu tiến độ Game 1 dùng chữ `#3D7962` trên nền `#EAFBF3`, đạt khoảng 4.77:1.
...
- Game 2 cấu hình mọi điểm khác biệt với đường kính vùng chạm tối thiểu 44 đơn vị trên viewBox 300×300; E2E quy đổi theo kích thước SVG render thật và yêu cầu đạt tối thiểu 44×44px trên iPhone/iPad.
...
- Game 6 giữ ô nhỏ nhất 48×48px ở vòng dày nhất 6 cột trên chiều rộng iPhone 390px; Game 7 chuyển bốn lựa chọn sang lưới 2 cột dưới 520px và các lựa chọn render đều đạt tối thiểu 44×44px.
- Game 8 giữ bốn phím hướng 72×72px; Game 9 và Game 10 dùng nút gameplay toàn chiều rộng với vùng chạm lớn hơn đáng kể so với ngưỡng 44×44px trên iPhone/iPad.

## Baseline tự động

`tests/accessibility-baseline.test.js` khóa các yêu cầu nền tảng trên cho toàn bộ 13 trang. `tests/live-region-accessibility.test.js` ngăn hồi quy đối với thông báo động dành cho trình đọc màn hình. `tests/game1-reward-live-region.test.js` khóa việc khai báo live region trực tiếp của phần thưởng Game 1; `e2e/game1-reward-live-region.spec.js` xác nhận thuộc tính render và việc không cướp focus. `tests/games2-4-reward-live-region.test.js` khóa việc khai báo live region trực tiếp của phần thưởng Game 2–4; `e2e/games2-4-reward-live-region.spec.js` xác nhận thuộc tính render và việc không cướp focus. `e2e/touch-targets.spec.js` đo kích thước thực tế của các nút điều khiển chung trên iPhone/iPad. `e2e/focus-visible.spec.js` xác nhận focus ring khi điều hướng bằng bàn phím. `tests/disabled-controls.test.js` kiểm tra trạng thái disabled qua hợp đồng tải chung của `shared-ui.js`. `e2e/text-contrast.spec.js` đo màu chữ và nền thực tế của các nút dùng chung. `tests/stat-label-contrast.test.js` khóa màu nhãn thống kê dùng chung ở mức tối thiểu 4.5:1; `e2e/stat-label-contrast.spec.js` đo màu render thực tế của nhãn đang hiển thị trong Game 1–10. `tests/instruction-text-contrast.test.js` khóa token chữ phụ trên cả nền kem và nền trắng; `e2e/instruction-text-contrast.spec.js` đo màu render thực tế của chữ hướng dẫn trong Game 1–10. `tests/game1-resume-contrast.test.js` khóa màu chữ phụ và hai nút trong hộp tiếp tục Game 1; `e2e/game1-resume-contrast.spec.js` đo màu render thực tế của cùng ba thành phần. `tests/completion-message-contrast.test.js` khóa selector và tỷ lệ tương phản của câu khen hộp hoàn thành; `e2e/completion-message-contrast.spec.js` đo màu render thực tế trong Game 1–10. `tests/game1-save-status-contrast.test.js` khóa màu thông báo lưu Game 1; `e2e/game1-save-status-contrast.spec.js` đo màu render thực tế. `tests/game1-keyboard-accessibility.test.js` khóa trạng thái ARIA và vòng đời lá bài Game 1. `e2e/games2-4-tab-order.spec.js` kiểm tra thứ tự Tab và việc bỏ qua phần tử ẩn/disabled của Game 2–4. `e2e/games5-7-tab-order.spec.js` kiểm tra thứ tự Tab tương đối và phần tử ẩn/disabled của Game 5–7. `e2e/games8-10-tab-order.spec.js` áp dụng cùng baseline cho Game 8–10. `tests/game2-keyboard-cursor.test.js` và `e2e/game2-keyboard-cursor.spec.js` khóa hợp đồng tải, phản hồi trực quan và thao tác bàn phím của Game 2. `tests/game3-keyboard-matching.test.js` và `e2e/game3-keyboard-matching.spec.js` khóa luồng chọn hình, chọn bóng, kết quả đúng/sai và trạng thái focus của Game 3. `tests/game4-keyboard-sorting.test.js` và `e2e/game4-keyboard-sorting.spec.js` khóa luồng chọn đồ vật, chọn giỏ, kết quả đúng/sai và trạng thái focus của Game 4. `tests/game2-gameplay-touch-targets.test.js` khóa bán kính vùng chạm trong dữ liệu Game 2; `e2e/game2-gameplay-touch-targets.spec.js` đo vùng chạm quy đổi theo kích thước SVG render thật. `tests/games3-4-gameplay-touch-targets.test.js` khóa kích thước CSS và lưới responsive; `e2e/games3-4-gameplay-touch-targets.spec.js` đo trực tiếp mục tiêu render thật bằng `boundingBox()`. `tests/game5-touch-target.test.js` khóa vòng chạm mở rộng và chuỗi tải offline; `e2e/game5-touch-target.spec.js` đo mọi chấm và kích hoạt gameplay từ phần nằm ngoài chấm nhìn thấy. `tests/games6-7-gameplay-touch-targets.test.js` khóa công thức kích thước ô Game 6 và lưới responsive Game 7; `e2e/games6-7-gameplay-touch-targets.spec.js` đo trực tiếp các mục tiêu đang hiển thị bằng `boundingBox()`. `tests/games8-10-gameplay-touch-targets.test.js` khóa kích thước CSS của phím hướng và hai nút gameplay; `e2e/games8-10-gameplay-touch-targets.spec.js` đo trực tiếp kích thước render thực tế trên iPhone/iPad.
...
1. Rà soát độ tương phản của nội dung nhỏ và vật thể gameplay còn lại.
2. Kiểm thử thực tế thông báo phần thưởng và auto-save với VoiceOver/TalkBack cho Game 5–10.
3. Mở rộng E2E accessibility cho các luồng tương tác hoàn chỉnh trên iPhone/iPad.
